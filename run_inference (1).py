...
post-processing pipeline, self-consistency majority voting, and
detailed error analysis.

Post-processing pipeline (applied in order to EACH completion)
--------------------------------------------------------------
...
---------------------------------
  SC_N completions are generated per question at temperature SC_TEMPERATURE.
  vLLM batches all n completions for the same prompt together (PagedAttention
  reuses the KV cache for the shared prompt prefix), so this is much cheaper
  than running inference SC_N times separately.

  After post-processing, the final \\boxed{} from each completion is normalised
  to a canonical vote key — \\frac{2}{6} and \\frac{1}{3} map to the same key,
  integers like 7, 7.0, 7.000000 collapse to "7", etc.  The precision-fixing
  step already runs before voting so equivalent decimals are aligned first.
  The plurality-winning completion is returned as the final response.

  Set SC_N = 1 to disable self-consistency and fall back to single-sample mode
  (temperature is still SC_TEMPERATURE in that case; set it to 0.0 for greedy).

Error categories tracked per response (union across all SC_N completions)
--------------------------------------------------------------------------
...
SC_N           = 10   # Completions per question.  Most of the gain over greedy
                       # comes from the first 10-20 samples (Wang et al. 2023);
                       # beyond 40 the improvement is marginal.  8-12 is the
                       # practical sweet spot.  Set to 1 to disable SC entirely.
SC_TEMPERATURE = 0.6  # 0.5-0.7 recommended for Qwen.  0.0 collapses to greedy.
...
def _to_canonical(s: str) -> str:
    """
    Convert a single sub-answer string to a canonical vote key.

    Rational forms are reduced via Python's Fraction so that \\frac{2}{6}
    and \\frac{1}{3} map to the same key ("1/3").  Plain integers are
    returned without a denominator ("7", not "7/1").  Decimals are rounded
    to 6 decimal places (matching the judger's round(gold, 6)) — note that
    the precision-fix step already runs BEFORE voting, so most
    low-precision decimals will have been extended or converted to fractions
    by the time this function is called.  Non-numeric strings fall through
    to a lowercase stripped form.
    """
    s = s.strip()
    ...
    # Plain fraction: num/den  (no backslash)
    m2 = re.fullmatch(r"(-?\d+)\s*/\s*(-?\d+)", s)
    if m2:
        try:
            frac = Fraction(int(m2.group(1)), int(m2.group(2)))
            if frac.denominator == 1:
                return str(frac.numerator)
            return f"{frac.numerator}/{frac.denominator}"
        except (ValueError, ZeroDivisionError):
    ...
    try:
        ...
        if val == int(val) and "." not in s:
            return str(int(val))        # e.g. "7", "-3"
        if val == int(val):
            return str(int(val))        # "7.0" → "7"
        return f"{round(val, 6):.6f}"   # round to judger precision
    except ValueError:
        pass
...
def _normalize_for_vote(boxed_content: str, is_mcq: bool) -> str:
    """
    Normalise a complete boxed-content string to a canonical vote key.

    MCQ:  extract letters, sort → "A,C,E"  (spacing-insensitive)
    Math: normalise each comma-separated sub-answer independently and join
          with " | ".  Order is preserved — "7, 3" and "3, 7" are different
          because for systems of equations the order is meaningful.
    """
    if is_mcq:
        letters = sorted(re.findall(r"[A-Za-z]", boxed_content))
        return ",".join(l.upper() for l in letters) or "__empty__"

    parts = [p.strip() for p in boxed_content.split(",")]
    return " | ".join(_to_canonical(p) for p in parts)


def _extract_final_boxed(response: str) -> Optional[str]:
    """
    Return the content of the first \\boxed{} that appears after </think>,
    or None if none is found.  (After apply_bandaid there should be exactly one.)
    """
    think_idx = response.rfind(_THINK_END)
    section   = response[think_idx + len(_THINK_END):] if think_idx >= 0 else response
    boxes     = _extract_boxed_contents(section)
    return boxes[0][2] if boxes else None
...
def majority_vote(
    ...
    """
    Select the best post-processed completion via plurality vote on the
    final \\boxed{} answer.

    Algorithm
    ---------
    1. Extract the final \\boxed{} content from each post-processed completion.
    2. Normalise each answer to a canonical key (handles \\frac{1}{3} == \\frac{2}{6},
       7 == 7.0, etc.).  Completions with no boxed answer vote for "__no_answer__".
    3. Count votes among valid (non-"__no_answer__") keys.
    4. Return the FIRST completion whose answer matches the plurality winner,
       plus a metadata dict with vote counts for diagnostics.

    Falls back to the first completion if every completion lacks a boxed answer.
    """
    normalized: list[str] = []
    for comp in processed_completions:
        content = _extract_final_boxed(comp)
        norm    = (_normalize_for_vote(content, is_mcq) if content is not None
                   else "__no_answer__")
        normalized.append(norm)

    vote_counts: Counter = Counter(normalized)
    valid_counts = {k: v for k, v in vote_counts.items() if k != "__no_answer__"}

    winner = max(valid_counts, key=valid_counts.get) if valid_counts else "__no_answer__"

    for comp, norm in zip(processed_completions, normalized):
        if norm == winner:
            return comp, {
                "sc_winner":     winner,
                "sc_votes":      vote_counts[winner],
                "sc_total":      len(processed_completions),
                "sc_valid":      sum(valid_counts.values()),
                "sc_candidates": len(valid_counts),
            }

    # Fallback — should not happen
    return processed_completions[0], {
        "sc_winner": winner, "sc_votes": 0,
        "sc_total": len(processed_completions), "sc_valid": 0, "sc_candidates": 0,
    }
...
def main():
    ...
    sampling_params = SamplingParams(
        ...
        n=SC_N,  # vLLM batches all SC_N completions together; prompt KV cache is computed once.
    )
    ...
    # ── Generate ─────────────────────────────────────────────────────────────
    print(f"Generating {len(prompts)} prompts × {SC_N} completions each...")
    outputs = llm.generate(prompts, sampling_params=sampling_params)
    # outputs[i].outputs is a list of SC_N RequestOutput objects.
    all_completions: list[list[str]] = [
        [o.text.strip() for o in out.outputs]
        for out in outputs
    ]

    # ── Post-process, vote & evaluate ────────────────────────────────────────
    print("Post-processing, voting, and scoring...")
    results     = []
    error_tally = defaultdict(int)
    sc_stats: Counter = Counter()   # unanimous / strong / weak / no_answer

    for item, completions_raw in tqdm(zip(data, all_completions), total=len(data)):
        is_mcq   = bool(item.get("options"))
        question = item["question"]

        # ── Diagnose all raw completions; union flags per question ────────────
        # Counts represent "questions where ≥1 completion had this error".
        question_flags: dict = defaultdict(bool)
        for raw in completions_raw:
            for flag, val in diagnose(raw, question).items():
                if val:
                    question_flags[flag] = True
        for flag, val in question_flags.items():
            if val:
                error_tally[flag] += 1

        # ── Post-processing pipeline on every completion ──────────────────────
        processed: list[str] = []
        for raw in completions_raw:
            ...
            if _THINK_END in response:
                ...
                response     = think_part + _THINK_END + answer_part

            processed.append(response)

        # ── Self-consistency majority vote ────────────────────────────────────
        kaggle_response, sc_info = majority_vote(processed, is_mcq)

        # Bucket the vote spread for reporting
        n_total = sc_info["sc_total"]
        n_votes = sc_info["sc_votes"]
        if sc_info["sc_valid"] == 0:
            sc_stats["no_answer"] += 1
        elif n_votes == n_total:
            sc_stats["unanimous"] += 1
        elif n_votes >= max(2, (n_total * 2) // 3):   # ≥ ⌊2/3⌋ of completions
            sc_stats["strong"] += 1
        else:
            sc_stats["weak"] += 1
        ...
        if DO_EVAL and "answer" in item:
            ...
            # Count mismatch check (post-fix, on winning completion)
    ...
    if DO_EVAL:
        ...
        # ── Self-consistency vote-spread breakdown ──────────────────────────
        print(f"  Self-consistency (n={SC_N}, T={SC_TEMPERATURE}):")
        print("  " + "-" * 36)
        sc_labels = [
            ("unanimous",  f"Unanimous       ({SC_N}/{SC_N})"),
            ("strong",      "Strong majority (≥2/3)         "),
            ("weak",        "Weak majority   (<2/3)         "),
            ("no_answer",   "No valid answer                "),
        ]
        for key, label in sc_labels:
            count = sc_stats.get(key, 0)
            print(f"  {label} {count:>4} ({count / total:.1%})")
        print("=" * 40 + "\n")
